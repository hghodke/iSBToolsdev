...
import tools.LevenbergMarquardt;
import ij.IJ;
import ij.ImagePlus;
import ij.Prefs;
...
public class PeakFitter implements ExtendedPlugInFilter, DialogListener {
	...
	public PeakFitter(boolean useDiscoidalAveraging, int innerRadius, int outerRadius, double threshold, double thresholdValue, int minimumDistance, int fitRadius, int imageWidth) {

		DiscoidalAveragingFilter filter = new DiscoidalAveragingFilter();
		filter.setCircleOffsets(imp.getWidth(), innerRadius, outerRadius);
		
		DiscoidalAveragingFilter discoidalAvgFilter = new DiscoidalAveragingFilter(imageWidth, innerRadius, outerRadius);
		peakFinder = new PeakFinder(useDiscoidalAveraging, discoidalAvgFilter, threshold, thresholdValue, minimumDistance);
		...
		if (isPreview) {
			...
			synchronized (table) {
				...
				for (int i = 0; i < fittedPeaks; i++) {
					...
					double[] parameters = fitParameters[i];
					double[] errors = fitErrors[i];
	...
	public boolean dialogItemChanged(GenericDialog dialog, AWTEvent e) {
		...
		peakFinder = new PeakFinder(useDiscoidalAveraging,
				...
				threshold, thresholdValue, minimumDistance);
	...
	public int showDialog(ImagePlus imp, String arg, PlugInFilterRunner pfr) {
		...
		dialog.addNumericField("Z_scale (nm)", zScale, 2);
