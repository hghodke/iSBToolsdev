...
public class Cell_Fluorescence implements PlugIn {
	...
	public void run(String arg0){
		...
		String roiFolder =  IJ.getDirectory("Provide the directory where ROIs are stored");
			if (roiFolder==null) return;	
			
		File controlFile = new File(csvFilename);
		ROIFile = new File(roiFolder);
		
		StatsFolder = iSBOps.checkCreateSubDir(controlFile.getParent(), "Stats");
	...
	private static void decision_tree(String[] labels) throws IOException {
		...
		ResultsTable MIN = new ResultsTable();
	  	...
	  	if( choice == -1){
	  		...
	  		List<String> listchannel = getSubFileList(table, "Channel",labels[choice], "SaveToPath");	
			...
			//System.out.println(listchannel.get(i) + "-------------Folder: "+ Folder);
		 ...
		 */
			    ...
			    for (String label : table.keySet()) {
			       ...
			       //System.out.println(label);
			        for (int k = 1; k<=stackSize; k++){
						  ...
						  // System.out.println(k + "-- "+ average[0] + "---- " + average[1] + "---- " + average[2] + "---- ");
						   //SaveStats in Files
						   /**
			...
			SUM.saveAs(StatsFolder + File.separator + labels[choice] +".Cell.SUM.txt");
			AVERAGE.saveAs(StatsFolder + File.separator +  labels[choice]  +".Cell.AVERAGE.txt" );
			AREA.saveAs(StatsFolder + File.separator +  labels[choice]  +".Cell.AREA.txt" );
			STDDEV.saveAs(StatsFolder + File.separator + labels[choice]  +".Cell.STDDEV.txt");
			MAX.saveAs(StatsFolder + File.separator +  labels[choice]  +".Cell.MAX.txt");
			MIN.saveAs(StatsFolder + File.separator + labels[choice] + ".Cell.MIN.txt"	);
